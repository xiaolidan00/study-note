...
- fetch url获取html,处理html,fetch加载css等资源,注入到web-component中，然后执行js，利用with语句作用域隔离
- 利用Proxy拦截器来操作全局变量window等
- 用map记录监听事件，卸载子应用的时候移除事件
...
- 观察者模式大多数时候是**同步**的，比如当事件触发，Subject就会去调用观察者的方法。而发布-订阅模式大多数时候是**异步**的（使用消息队列）
...
[2024年3月前端面试记录：裸辞、旅游、面试](https://juejin.cn/post/7351321244124676106)
[23届前端,数字马力,3面面经](https://juejin.cn/post/7354724238581153827)
...
- tree层级，同级比较
- component层级，删除后重建
- element层级，key标识，节点操作插入，删除，移动

# Immer实现原理

<https://juejin.cn/post/6926099651510665230>

- 利用Proxy拦截器set的时候浅拷贝，并且在get的时候递归每层添加Proxy
...
[图片转shader](https://github.com/rkibria/img2shadertoy)
